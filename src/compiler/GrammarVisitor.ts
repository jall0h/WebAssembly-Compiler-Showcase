...
export default class GrammarVisitor<Result> extends ParseTreeVisitor<Result> {
  visit_node(tree: ParseTree, ts: type_env): Result {
    if (tree instanceof ExpContext && this.visitExp) {
      return this.visitExp(tree, ts);
    }
    if (tree instanceof MContext && this.visitM) {
      return this.visitM(tree, ts);
    }
    if (tree instanceof TContext && this.visitT) {
      return this.visitT(tree, ts);
    }
    if (tree instanceof FContext && this.visitF) {
      return this.visitF(tree, ts);
    }
    if (tree instanceof BexpContext && this.visitBexp) {
      return this.visitBexp(tree, ts);
    }
    if (tree instanceof TaContext && this.visitTa) {
      return this.visitTa(tree, ts);
    }
    if (tree instanceof AContext && this.visitA) {
      return this.visitA(tree, ts);
    }
    if (tree instanceof DefnContext && this.visitDefn) {
      return this.visitDefn(tree, ts);
    }
    if (tree instanceof ProgContext && this.visitProg) {
      return this.visitProg(tree, ts);
    }
    if (tree instanceof BlockContext && this.visitBlock) {
      return this.visitBlock(tree, ts);
    }
    if (tree instanceof ArgContext && this.visitArg) {
